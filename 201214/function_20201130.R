...
#### segmentation ####
segmentation <- function(Country,criteria=0.1, 
  ...
  tryCatch(
    ...
    error = function(e){
      ...
      if(length(coef_result[,1])>=3){
        pred_y = predict(fit32,newdata=x3)
        lines(x3+(b[3]-1), pred_y, col=4,lwd=2)
      }
      
      abline(v=b[-1],lty=2)
      legend("topleft",c("data","fit1","fit2","fit3"),pch=c(1,-1,-1,-1),lty=c(0,1,1,1),col=c(1,2,3,4),lwd=c(1,2,2,2))
      if(save_image){dev.off()}
...
segBertalanffy = function(Country,start_date=1,
                       ...
                       prediction_plot = FALSE,
                       daily_plot = FALSE){
  if(save_image){png(filename=paste0("plot/growthcurve/Bertalanffy/",Country,"_Bertalanffy.png"))}
  ...
  plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
       sub="Simple Moving Avarage (window size = 7)",
       ylab="Cumulative Cases", main=paste0(Country, " / Bertalanffy")) 
  ...
  tryCatch(
    expr = {
      ...
      fit11 = try(nls2(y_ ~ a1*((1-exp(-b1*x1))^c1), start = st,
                       algorithm = "plinear-random",control = nls.control(maxiter = max_iter)))
      initial_list = data.frame(a1=coef(fit11)[1],b1=coef(fit11)[2],c1=coef(fit11)[3])
      as.list(initial_list)
      fit12 = try( nls(y_ ~ a1*((1-exp(-b1*x1))^c1), start = initial_list))
      coef_result = data.frame(a=coef(fit12)[1],b=coef(fit12)[2],c=coef(fit12)[3])
      ...
      if(m>=2){
        ...
        st = data.frame(a2 = c(0.1*max(y), max(y)*2), b2 = c(0, 25), c2 = c(0, 0.5)) 
        fit21 = try(nls2(y_ ~ y_[1] + a2*((1-exp(-b2*x2))^c2), start = st,
                         algorithm = "plinear-random",control = nls.control(maxiter = max_iter)))
        initial_list = data.frame(a2=coef(fit21)[1], b2=coef(fit21)[2], c2=coef(fit21)[3])
        as.list(initial_list)
        fit22 = try( nls(y_ ~ y_[1] + a2*((1-exp(-b2*x2))^c2), start = initial_list) )
      ...
      if(m>=3){
        ...
        st = data.frame(a3 = c(0.1*max(y), max(y)*2), b3 = c(0, 25), c3 = c(0, 0.5)) 
        fit31 = try(nls2(y_ ~ y_[1] + a3*((1-exp(-b3*x3))^c3), start = st,
                         algorithm = "plinear-random",control = nls.control(maxiter = max_iter)))
        initial_list = data.frame(a3=coef(fit31)[1], b3=coef(fit31)[2], c3=coef(fit31)[3])
        as.list(initial_list)
        fit32 = try( nls(y_ ~ y_[1] + a3*((1-exp(-b3*x3))^c3), start = initial_list) )
  ...
  tryCatch(
    expr = {
      ...
      if(m>=3){
        ...
        st = data.frame(a3 = c(0.1*max(y), max(y)*2), b3 = c(0, 25), c3 = c(0, 0.5)) 
        fit31 = try(nls2(y_ ~ y_[1] + a3*exp(-b3*exp(-c3*x3)), start = st,
                         algorithm = "plinear-random",control = nls.control(maxiter = max_iter)))
        initial_list = data.frame(a3=coef(fit31)[1], b3=coef(fit31)[2], c3=coef(fit31)[3])
        as.list(initial_list)
        fit32 = try( nls(y_ ~ y_[1] + a3*exp(-b3*exp(-c3*x3)), start = initial_list) )
