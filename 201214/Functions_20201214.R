...
segmentation <- function(Country,criteria=0.1, 
  ...
  for(i in Country){
    ...
    if(save_image){jpeg(filename=paste0("plot/segmentation/image/",i,".jpeg"), width = 8, height = 8, units = 'in',res=600)}
    par(mfrow=c(4,1),mar= c(4, 5, 4, 2) + 0.2)
...
segLogistic = function(Country,start_date=1,
                       ...
                       daily_plot = FALSE){
  if(save_image){jpeg(filename=paste0("plot/growthcurve/Logistic/",Country,"_Logistic.jpeg"), width = 8, height = 8, units = 'in',res=600)}
  ...
  tryCatch(
    ...
    error = function(e){
      ...
      if(length(coef_result[,1])>=1){
        ...
        pred_Y = diff(pred_y)
        MSSE = sum( ((Y[2:length(x1)] - pred_Y)/mean(Y[2:length(x1)]))^2 ) # mean squared scaled error
        n_msse = length(x1)-1
      }
      if(length(coef_result[,1])>=2){
        ...
        pred_Y = diff(pred_y)
        idx_msse = (length(x1)+1) : (length(x1)+length(x2))
        MSSE = MSSE +
          sum( ( (Y[idx_msse] - pred_Y)/mean(Y[idx_msse]) )^2 ) # mean squared scaled error
        n_msse = n_msse + length(x2)        
      }
      
      if(length(coef_result[,1])>=1){
        cat("\nThe number of segment : ", length(coef_result[,1]), "\n")
        MSSE_result = data.frame(MSSE = MSSE/n_msse)
        cat(paste("MSSE : ",MSSE/n_msse, " \n"))
      }
      ...
      try(
        expr={
          ...
          print(coef_result)
          return(list(coef_result,MSSE_result))
        }
      )
    }
  )
...
segGompertz = function(Country,start_date=1,
                       ...
                       prediction_plot = FALSE,
                       daily_plot = FALSE){
  if(save_image){jpeg(filename=paste0("plot/growthcurve/Gompertz/",Country,"_Gompertz.jpeg"), width = 8, height = 8, units = 'in',res=600)}
